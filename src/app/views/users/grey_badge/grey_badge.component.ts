...
@Component({
  selector: 'app-grey_badge',
  templateUrl: './grey_badge.component.html',
  styleUrls: ['./grey_badge.component.scss']
})
export class GreyComponent implements OnInit {
  ...
  searchUser(data: any) {
    ...
    this._user.searchUser(this.search_count, data, this.access_token, 1,10)
      .then((data: any) => {
        console.log(data)
        if (data.status == 401) {
  ...
  getUsers() {
    this._user.getUserByBadge(this.count, 1)
      .then((data: any) => {
